...
import pages.GasStationPage;

public class GasStationPageStepDefinition {

	private WebDriver driver;
	private GasStationPage GasStationPageInstance;
	...
	public void user_lands_on_a_particular_page() {
		...
		this.driver.get("https://ethwdashboard.xyz/gas-tracker"); // Launch home page of the Ethereum Wallet Dash board

		this.driver.manage().window().maximize(); // Maximize window
		this.driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(5));
		
		GasStationPageInstance = new GasStationPage(this.driver);
		GasStationPageInstance.getSidebarSelectionElements().get(7).click(); // Click the seventh link in the side bar to get to Gas Prices Page
		
		String gasInformationTitle = GasStationPageInstance.getGasInformationTitleElement().getText();
		String generalMetricsTitle = GasStationPageInstance.getGeneralMetricsTitleElement().getText();
		String blockPricesTitle = GasStationPageInstance.getBlockPricesTitleElement().getText();
		String baseFeesTitle = GasStationPageInstance.getBaseFeesTitleElement().getText();
		
		Assert.assertEquals("https://ethwdashboard.xyz/gas-tracker", this.driver.getCurrentUrl());
		Assert.assertEquals("Gas Information", gasInformationTitle);
		Assert.assertEquals("General Metrics", generalMetricsTitle);
		Assert.assertEquals("Block Prices", blockPricesTitle);
		Assert.assertEquals("Base Fees", baseFeesTitle);
	}
	...
	@When("User reads gas prices")
	public void User_reads_gas_prices() {
		String gasInformationTitle = GasStationPageInstance.getGasInformationTitleElement().getText();
		String generalMetricsTitle = GasStationPageInstance.getGeneralMetricsTitleElement().getText();
		String blockPricesTitle = GasStationPageInstance.getBlockPricesTitleElement().getText();
		String baseFeesTitle = GasStationPageInstance.getBaseFeesTitleElement().getText();
	...
	@And("User selects go to dashboard button")
	public void User_selects_go_to_dashboard_button() {
		GasStationPageInstance.clickDashboardButtonElement();
	}
	...
	public void user_selects_a_certain_link() {
		...
		int randomNumber = (int) ((Math.random())*GasStationPageInstance.getSidebarSelectionElements().size() - 1);
		GasStationPageInstance.getSidebarSelectionElements().get(randomNumber).click();
	}
	...
	public void user_selects_navbar_to_enter_an_incorrect_wallet_address(String address) {
		GasStationPageInstance.setNavbarInputElement(address);
		GasStationPageInstance.clickNavbarButtonElement();
	}
	...
	public void user_selects_a_footer_link() {
		// Randomize selection on each test run
		int randomNumber = (int) ((Math.random())*GasStationPageInstance.getFooterLinkElements().size() - 1); 
		GasStationPageInstance.getFooterLinkElements().get(randomNumber).click();
	}
	...
	public void display_alert_warning_user() {
		String text = GasStationPageInstance.getNavbarAlertElement().getText();
